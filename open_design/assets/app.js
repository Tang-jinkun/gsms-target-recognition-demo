/* GSMS shared — lucide-style inline icons, top nav, toast, modal helpers */
(function () {
  // ---- icon path data (lucide, stroke-based) ----
  const I = {
    ...
    'folder-plus': '<path d="M4 20h16a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.93a2 2 0 0 1-1.66-.9l-.82-1.2A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13c0 1.1.9 2 2 2Z"/><line x1="12" x2="12" y1="10" y2="16"/><line x1="9" x2="15" y1="13" y2="13"/>',
  ...
  // ---- shared top nav ----
  const NAV = [
    { id: 'workbench', label: '场景', href: 'scenes.html' },
    { id: 'data',      label: '数据管理', href: 'data-hub.html' },
    { id: 'skills',    label: 'Skills', href: 'skills.html' },
    { id: 'settings',  label: '设置', href: 'settings.html' }
  ];
  window.renderTopnav = function (active) {
    const el = document.querySelector('[data-topnav]');
    if (!el) return;
    el.innerHTML =
      '<a class="brand" href="index.html"><span class="logo">G</span>GSMS</a>' +
      '<nav class="nav-links">' +
        NAV.map(n => '<a href="' + n.href + '"' + (n.id === active ? ' class="active"' : '') + '>' + n.label + '</a>').join('') +
      '</nav>' +
      '<div class="nav-right">' +
        '<span class="status-dot" title="后端在线"><span class="dot"></span>后端在线</span>' +
        '<span class="user-chip"><span class="avatar">LZ</span><span class="uname">李泽 · 研究员</span></span>' +
      '</div>';
  };
  ...
  window.toast = function (msg, kind) {
    ...
    t.innerHTML = icon(kind === 'error' ? 'alert-circle' : 'check') + '<span>' + msg + '</span>';
    t.querySelector('.ic').style.color = kind === 'error' ? 'var(--danger)' : 'var(--ok)';
    requestAnimationFrame(() => t.classList.add('show'));
    clearTimeout(t._t); t._t = setTimeout(() => t.classList.remove('show'), 2200);
  };

  // ---- icon hydration: any <i data-ic="name"></i> ----
  window.hydrateIcons = function (root) {
    (root || document).querySelectorAll('[data-ic]').forEach(n => {
      n.outerHTML = icon(n.getAttribute('data-ic'), n.getAttribute('data-cls') || '');
    });
  };
})();
